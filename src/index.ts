...
async function getSkillDetail(
  args: z.infer<typeof GetSkillDetailSchema>
): Promise<string> {
...
async function browseCatalog(
  args: z.infer<typeof BrowseCatalogSchema>
): Promise<string> {
...
async function recommendSkills(
  args: z.infer<typeof RecommendSkillsSchema>
): Promise<string> {
  // Use new publicRecommend API with MMR
  const recommendations = await (client as any).publicRecommend({
    query: args.context,
  ...
  const filtered = recommendations
    .filter((r: typeof recommendations[0]) => !args.current_skills.includes(r.id) && !args.current_skills.includes(r.slug))
    .slice(0, args.limit);
  ...
  const results = filtered
    .map((s: typeof filtered[0], i: number) => {
      const score = s.simple_score ? `Score: ${s.simple_score}` : "";
      const category = s.category ? `[${s.category}]` : "";
...
async function getRecentSkills(): Promise<string> {
  ...
  return {
    tools: [
      ...
      },
      {
        name: "install_skill",
        description:
          `Install a skill to the local filesystem. First call shows a preview, then call with confirm=true to install. Auto-detects current CLI environment (detected: ${DETECTED_AGENT}).`,
        inputSchema: {
          type: "object",
          properties: {
            skill_id: {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  // Track MCP tool call
  const trackMCPCall = async (success: boolean, error?: string, metadata?: Record<string, unknown>) => {
    try {
